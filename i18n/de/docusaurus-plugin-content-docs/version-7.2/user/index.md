---
sidebar_position: 1
title: Willkommen
description: Willkommen
---

<!-- markdownlint-disable MD041 -->

import OcLogoPetrol from '/static/img/oc-logo-petrol.svg';
import OcLogoLilac from '/static/img/oc-logo-lilac.svg';

<!-- markdownlint-enable MD041 -->

# Willkommen

<OcLogoPetrol width="250" height="55" className="light-mode-image"/>
<OcLogoLilac width="250" height="55" className="dark-mode-image"/>

## Willkommen zur OpenCloud-Wissensdatenbank

Diese Dokumentation hilft Ihnen beim Einstieg in OpenCloud, beim Verständnis der wichtigsten Funktionen und bei der Suche nach Hilfen für den Alltag, die Administration und die Fehlersuche.

## Was ist OpenCloud?

OpenCloud ist eine File-Sharing- und Kollaborationsplattform der [Heinlein Group](https://www.heinlein.group/). Sie hilft Teams dabei, Dateien geräteübergreifend zu speichern, zu organisieren, zu teilen und gemeinsam zu bearbeiten.

Je nach Arbeitsweise können Sie OpenCloud auf unterschiedliche Weise nutzen:

- Der [Web Client](./general/) bietet direkten Zugriff auf alle OpenCloud-Funktionen im Browser, einschließlich Dateiverwaltung, Kollaborationsfunktionen und Verwaltungsoptionen, soweit Ihre Berechtigungen dies zulassen.

- Der [Desktop Client](./desktop-client/) synchronisiert Dateien zwischen OpenCloud und Ihrem lokalen Computer für die Offline-Arbeit und automatische Aktualisierungen.

- Die [iOS App](./ios-app/) bietet bequemen Zugriff auf Dateien und wichtige Aktionen unterwegs.

- Die [Android App](./android-app/) bietet bequemen Zugriff auf Dateien und wichtige Aktionen unterwegs.

- Die [Collabora Online-Integration](./collabora) ermöglicht das Bearbeiten von Office-Dokumenten direkt im Browser und die Zusammenarbeit in Echtzeit.

Jeder Client und jede Integration ist für einen bestimmten Anwendungsfall ausgelegt: der Web Client für den umfangreichen Zugriff im Browser, der Desktop Client für die lokale Synchronisierung, die mobilen Apps für den Zugriff unterwegs und Collabora für die Dokumentbearbeitung im Browser.

## Administration und Bereitstellung

Wenn Sie Ihre eigene OpenCloud-Umgebung installieren, einrichten oder verwalten möchten, finden Sie Hilfe in der [Admin-Dokumentation](../admin/) und im Abschnitt [Erste Schritte](../admin/getting-started/).

## Dokumentationsversionen

Diese Dokumentation ist versioniert, damit Sie die Informationen passend zu Ihrer OpenCloud-Version finden.

- **Rolling-Version**  
  Entspricht dem aktuellen Entwicklungsstand. Änderungen werden fortlaufend eingepflegt, einzelne Funktionen sind möglicherweise noch nicht vollständig dokumentiert.

- **Stable-Release-Versionen**  
  Dokumentation zu offiziell veröffentlichten Versionen. Diese Versionen werden geprüft und bleiben nach der Veröffentlichung unverändert.

Über das Versionsmenü oben rechts können Sie zwischen der Rolling-Version und bestimmten Release-Versionen wechseln.

## Dokumentation entdecken

### Apps

- [OpenCloud Desktop Client](./desktop-client/)  
  Hier erfahren Sie, wie Sie den Desktop Client installieren, einrichten und verwenden.

- [OpenCloud iOS App](./ios-app/)  
  Hier finden Sie Hinweise zur Nutzung von OpenCloud auf iPhone und iPad.

- [OpenCloud Android App](./android-app/)  
  Hier finden Sie Hinweise zur Nutzung von OpenCloud auf Android-Geräten.

### Web und Bearbeitung

- [Benutzeroberfläche und Navigation](./general/)  
  Hier lernen Sie den Web Client kennen, passen Kontoeinstellungen an und verändern die Darstellung.

- [Collabora Online](./collabora)  
  Hier erfahren Sie, wie Sie Dokumente direkt im Browser bearbeiten.

- [Dateien und Ordner verwalten](./files-and-folders/)  
  Hier finden Sie Hinweise zum Erstellen, Organisieren, Verschieben, Löschen und Wiederherstellen von Dateien und Ordnern.

- [Dateien und Ordner teilen](./sharing/)  
  Hier erfahren Sie, wie Sie Inhalte intern teilen, öffentliche Links erstellen und File Drop verwenden.

- [Im Team mit Spaces arbeiten](./spaces/)  
  Hier erfahren Sie, wie Sie Spaces für die Zusammenarbeit in Teams nutzen.

### Administration

- [Rollen](./roles/)  
  Hier erfahren Sie mehr über Benutzerrollen, Space-Rollen und Freigaberechte in OpenCloud.

- [Administration von OpenCloud](./admin/)  
  Hier finden Sie Informationen zu Einstellungen, App-Tokens und WebDAV-Zugriff.

### Hilfe

- [Häufige Probleme und Hilfe](./common-issues)  
  Hier finden Sie Lösungen für häufige Probleme und Antworten auf häufig gestellte Fragen.
