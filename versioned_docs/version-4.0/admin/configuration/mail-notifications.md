...
See section [Environment Variables](https://docs.opencloud.eu/docs/dev/server/services/notifications/environment-variables/) in the developers documentation for possible values for these variables. Note, that the environment variables are prefixed with `NOTIFICATIONS_`, so `NOTIFICATIONS_SMTP_AUTHENTICATION` shows possible values for `SMTP_AUTHENTICATION` in the `.env` file.
