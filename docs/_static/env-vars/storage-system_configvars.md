...
|`OC_CACHE_ENABLE_TLS`<br/>`STORAGE_SYSTEM_CACHE_ENABLE_TLS`| 7.3.0 |bool|`Enable TLS for the connection to file metadata cache.`|`"false"`|
|`OC_INSECURE`<br/>`OC_CACHE_TLS_INSECURE`<br/>`STORAGE_SYSTEM_CACHE_TLS_INSECURE`| 7.3.0 |bool|`Whether to verify the server TLS certificates.`|`"false"`|
|`OC_CACHE_TLS_ROOT_CA_CERTIFICATE`<br/>`STORAGE_SYSTEM_CACHE_TLS_ROOT_CA_CERTIFICATE`| 7.3.0 |string|`The root CA certificate used to validate the server's TLS certificate. If provided STORAGE_SYSTEM_CACHE_TLS_INSECURE will be seen as false.`|`""`|
