...
In order to do so a POST request is sent to the `generate-invite` endpoint of the sciencemesh API. The generated token is passed on to the receiver, who will then use the `accept-invite` endpoint to accept the invitation. As a result remote users will be added to the `ocminvitemanager` on both sides. See [invitation flow](https://github.com/opencloud-eu/opencloud/blob/main/services/ocm/invitation-flow) for the according sequence diagram.
...
See [Create share flow](https://github.com/opencloud-eu/opencloud/blob/main/services/ocm/create-share-flow) for the according sequence diagram.
