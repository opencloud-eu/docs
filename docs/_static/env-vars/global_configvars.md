...
| `OC_CACHE_TLS_ROOT_CA_CERTIFICATE` | 7.3.0 | string | The root CA certificate used to validate the server's TLS certificate. If provided GRAPH_CACHE_TLS_INSECURE will be seen as false. | "" |
...
| `OC_MACHINE_AUTH_API_KEY` | 7.5.0 | string | Machine auth API key used to validate internal requests necessary for the access to resources from other services. | "" |
