...
|`PROXY_TRANSPORT_CLIENT_DIAL_TIMEOUT`| 7.5.0 |Duration|`Dial timeout for the HTTP client.`|`"30s"`|
|`PROXY_TRANSPORT_CLIENT_DIAL_KEEP_ALIVE`| 7.5.0 |Duration|`Dial keep alive for the HTTP client.`|`"30s"`|
|`PROXY_TRANSPORT_CLIENT_FORCE_ATTEMPT_HTTP2`| 7.5.0 |bool|`Force attempt HTTP/2 for the HTTP client. Set to true if backend communication is encrypted and HTTP/2 is desired.`|`"false"`|
|`PROXY_TRANSPORT_CLIENT_MAX_IDLE_CONNS`| 7.5.0 |int|`Maximum idle connections for the HTTP client.`|`"100"`|
|`PROXY_TRANSPORT_CLIENT_MAX_IDLE_CONNS_PER_HOST`| 7.5.0 |int|`Maximum idle connections per host for the HTTP client.`|`"100"`|
|`PROXY_TRANSPORT_CLIENT_IDLE_CONN_TIMEOUT`| 7.5.0 |Duration|`Idle connection timeout for the HTTP client.`|`"1m30s"`|
|`PROXY_TRANSPORT_CLIENT_TLS_HANDSHAKE_TIMEOUT`| 7.5.0 |Duration|`TLS handshake timeout for the HTTP client.`|`"10s"`|
|`PROXY_TRANSPORT_CLIENT_EXPECT_CONTINUE_TIMEOUT`| 7.5.0 |Duration|`Expect continue timeout for the HTTP client.`|`"1s"`|
...
|`OC_CACHE_ENABLE_TLS`<br/>`PROXY_OIDC_USERINFO_CACHE_ENABLE_TLS`| 7.3.0 |bool|`Enable TLS for the connection to file metadata cache.`|`"false"`|
|`OC_INSECURE`<br/>`OC_CACHE_TLS_INSECURE`<br/>`PROXY_OIDC_USERINFO_CACHE_TLS_INSECURE`| 7.3.0 |bool|`Whether to verify the server TLS certificates.`|`"false"`|
|`OC_CACHE_TLS_ROOT_CA_CERTIFICATE`<br/>`PROXY_OIDC_USERINFO_CACHE_TLS_ROOT_CA_CERTIFICATE`| 7.3.0 |string|`The root CA certificate used to validate the server's TLS certificate. If provided PROXY_OIDC_USERINFO_CACHE_TLS_INSECURE will be seen as false.`|`""`|
...
|`PROXY_TRANSFER_TIMEOUT`| 7.5.0 |Duration|`The storage transfer timeout.`|`"24h0m0s"`|
