...
|`PROXY_TRANSPORT_CLIENT_DIAL_TIMEOUT`| %NEXT% |Duration|`Dial timeout for the HTTP client.`|`"30s"`|
|`PROXY_TRANSPORT_CLIENT_DIAL_KEEP_ALIVE`| %NEXT% |Duration|`Dial keep alive for the HTTP client.`|`"30s"`|
|`PROXY_TRANSPORT_CLIENT_FORCE_ATTEMPT_HTTP2`| %NEXT% |bool|`Force attempt HTTP/2 for the HTTP client. Set to true if backend communication is encrypted and HTTP/2 is desired.`|`"false"`|
|`PROXY_TRANSPORT_CLIENT_MAX_IDLE_CONNS`| %NEXT% |int|`Maximum idle connections for the HTTP client.`|`"100"`|
|`PROXY_TRANSPORT_CLIENT_MAX_IDLE_CONNS_PER_HOST`| %NEXT% |int|`Maximum idle connections per host for the HTTP client.`|`"100"`|
|`PROXY_TRANSPORT_CLIENT_IDLE_CONN_TIMEOUT`| %NEXT% |Duration|`Idle connection timeout for the HTTP client.`|`"1m30s"`|
|`PROXY_TRANSPORT_CLIENT_TLS_HANDSHAKE_TIMEOUT`| %NEXT% |Duration|`TLS handshake timeout for the HTTP client.`|`"10s"`|
|`PROXY_TRANSPORT_CLIENT_EXPECT_CONTINUE_TIMEOUT`| %NEXT% |Duration|`Expect continue timeout for the HTTP client.`|`"1s"`|
