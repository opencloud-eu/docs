...
|---|---|---|:---|
| WEB_OPTION_RUNNING_ON_EOS is deprecated and will be removed in a future release. | 6.2.0 | next-prod |  |
...
|`WEB_STORE_DATABASE`| 7.4.0 |string|`The bucket name the store should use.`|`"web"`|
|`OC_PERSISTENT_STORE_AUTH_USERNAME`<br/>`WEB_STORE_AUTH_USERNAME`| 7.4.0 |string|`The username to authenticate with the store.`|`""`|
|`OC_PERSISTENT_STORE_AUTH_PASSWORD`<br/>`WEB_STORE_AUTH_PASSWORD`| 7.4.0 |string|`The password to authenticate with the store.`|`""`|
|`OC_PERSISTENT_STORE_ENABLE_TLS`<br/>`WEB_STORE_ENABLE_TLS`| 7.4.0 |bool|`Enable TLS for the connection to the store.`|`"false"`|
|`OC_INSECURE`<br/>`OC_PERSISTENT_STORE_TLS_INSECURE`<br/>`WEB_STORE_TLS_INSECURE`| 7.4.0 |bool|`Whether to verify the server TLS certificates.`|`"false"`|
|`OC_PERSISTENT_STORE_TLS_ROOT_CA_CERTIFICATE`<br/>`WEB_STORE_TLS_ROOT_CA_CERTIFICATE`| 7.4.0 |string|`The root CA certificate used to validate the server's TLS certificate. If provided WEB_STORE_TLS_INSECURE will be seen as false.`|`""`|
