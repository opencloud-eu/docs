...
- Not portable: Symlinks often point to paths that only exist on the original machine. On another device, the target path likely does not exist.
...
## Files with "~$" in the name are temporary lock files

The OpenCloud Desktop Client does not synchronize files starting with `~$`, such as `~$document.docx`.
These files are temporary lock files created by Microsoft Office applications while a document is open.
...
They are not actual content files. Instead, they act as internal markers so Office can prevent multiple users from editing the same document at the same time.
Once the file is closed, Office removes the `~$` file automatically.

For a more detailed explanation of locked files in OpenCloud, see [Locked files](./locked-files.md).
